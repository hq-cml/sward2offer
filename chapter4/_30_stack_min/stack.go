...
//思路：
//引入辅助栈，永远保存当前最小元素
//方案1：入栈的时候进行检测，如果入栈的元素比辅助栈top还小，则辅助栈入栈该元素，否则入栈当前top元素；出栈就是主栈和辅助栈无脑出栈即可。简单但浪费空间
//方案2：入栈的时候进行检测，如果入栈的元素<=辅助栈top，则辅助栈入栈该元素，否则不入辅助栈；出栈就是主栈出栈，如果出栈元素==辅助栈top，则辅助栈出栈。省空间
//难度：3*
