...
func PrintProbability(n int) {
	...
	//如果只有一个骰子，那么出现次数为1（第一轮）
	for i := 1; i <= DiceMaxNumber; i++ {
		probablities[idxOrg][i] = 1
	}
	...
	for k := 2; k <= n; k++ {
		idxNew := 1 - idxOrg
		for i := 0; i < k; i++ {
			probablities[idxNew][i] = 0 //这些都不可能出现，因为k个骰子最小也是k
		}
		...
		for i := k; i <= DiceMaxNumber*k; i++ {
			...
			//并且，i-j需要保证>=0，否则也不能参与计算（其实更严谨的应该是i-j > k-1，只不过小的位数都是0，也无所谓)
			for j := 1; j <= i && j <= DiceMaxNumber; j++ {
				probablities[idxNew][i] += probablities[idxOrg][i-j]
			}
		}

		idxOrg = idxNew //数组索引来回交替
	}

	//打印所有的概率
	total := math.Pow(DiceMaxNumber, float64(n))
	for i := n; i <= n*DiceMaxNumber; i++ {
		ratio := float64(probablities[idxOrg][i]) * 100 / total
		fmt.Printf("%v 出现次数：%v, 概率：%2.2f %%\n", i, probablities[idxOrg][i], ratio)
	}
}
