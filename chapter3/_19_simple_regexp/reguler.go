 ...
 * 和"ab*ac*a"匹配，但与模式"aa.a"及"ab*a"均不匹配。
 *
 * PS：本题是LeetCode 10
 */
...
func Regexp(str, pattern string) bool {
	...
	if len(pattern) > 1 && pattern[1] == '*' {
		...
		if str[0] == pattern[0] || // 如果第一个字符相等
			...
			// return一个或的关系！
			return Regexp(str[1:], pattern[2:]) || // 字符串匹配一个，模式串跳过*
				Regexp(str[1:], pattern) || // 字符串匹配一个，模式串维持原状，因为*可能表示n个字符
				Regexp(str, pattern[2:]) // 字符串维持，模式串直接忽略*和其前面字符，因为*也可能标识0次匹配
		} else {
			// 第一个字符不相等，则直接利用*表示0个字符的功效，忽略后平移模式串
		...
		if len(str) > 0 && len(pattern) > 0 { // 必须保证此时字符串和模式串都未完结
			if str[0] == pattern[0] || // 如果第一个字符相等
				...
				return Regexp(str[1:], pattern[1:]) // 同时向后平移
			}
		}
	}
