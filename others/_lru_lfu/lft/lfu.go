...
type LFUCache struct {
	Cap         int           // 容量
	KeyToValue  map[int]*Node // 实际的缓存key=>Node
...
func (lf *LFUCache)Put(key,val int) int{
	//key 存在，需要更新freqMap
	if _,exist := lf.KeyToValue[key]; exist{
		//更新最新的值
		lf.KeyToValue[key].value = val
		//更新频率
		lf.incrFreq(key)
	} else {
		//如果不存在, 需要判断容量是不是满了
		//如果容量满了，需要根据最小的minFreq，先删除一个key
		if len(lf.KeyToValue) >= lf.Cap {
